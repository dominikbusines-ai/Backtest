...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  try {
    ...
    const description = `${trade.result_type === "no_trade" ? "Kein Trade" : trade.result_r === null ? "Backtest" : `${trade.result_r}R`} · ${trade.tags.map((tag) => tag.name).slice(0, 3).join(", ") || "ohne Tags"}`;
