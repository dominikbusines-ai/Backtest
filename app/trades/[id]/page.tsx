...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  try {
    ...
    const description = `${trade.trade_mode === "live" ? "Live Trade" : "Backtest"} · ${trade.result_type === "no_trade" ? "Kein Trade" : trade.result_r === null ? "ohne R-Ergebnis" : `${trade.result_r}R`}`;
...
export default async function TradeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell>
    ...
    {trade.pre_trade_assessment && <section className="panel mb-6 p-4 sm:p-5"><h2 className="text-sm font-semibold">Pre-Trade-Bewertung</h2><p className="mt-2 text-xs text-zinc-500">Was brauche ich, damit ich vom Trend überzeugt bin?</p><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-6 text-zinc-300">{trade.pre_trade_assessment}</p></section>}
    <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
      <section className="panel overflow-hidden">{trade.screenshot_signed_url ? <TradeImageViewer src={trade.screenshot_signed_url} alt={`Chart zu ${trade.instrument}`} /> : <div className="flex min-h-52 items-center justify-center text-sm text-zinc-700 sm:min-h-72">Kein Screenshot</div>}</section>
      <div className="space-y-6"><section className="panel p-4 sm:p-5"><div className="grid grid-cols-2 gap-x-4 gap-y-4 sm:gap-x-5">{details.map(([label,value]) => <div key={label} className="min-w-0"><span className="label">{label}</span><p className="break-words text-sm font-medium text-zinc-200">{String(value)}</p></div>)}</div></section>{(trade.context || trade.entry_note) && <section className="panel space-y-5 p-4 sm:p-5">{trade.context && <div><span className="label">Marktkontext</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.context}</p></div>}{trade.entry_note && <div><span className="label">{trade.result_type === "no_trade" ? "Warum kein Entry" : "Warum Entry?"}</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.entry_note}</p></div>}</section>}{(trade.review_observation || trade.review_mistake || trade.review_invalidation || trade.review_illogical) && <section className="panel space-y-5 p-4 sm:p-5"><p className="text-sm font-semibold">Nachträgliche Auswertung</p>{trade.review_observation && <div><span className="label">Aufgefallen</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_observation}</p></div>}{trade.result_type === "no_trade" ? trade.review_illogical && <div><span className="label">Was erschien unlogisch</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_illogical}</p></div> : <>{trade.review_mistake && <div><span className="label">Missachtet</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_mistake}</p></div>}{trade.review_invalidation && <div><span className="label">Entry entkräftet durch</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_invalidation}</p></div>}</>}</section>}</div>
    </div>
  </AppShell>;
}
