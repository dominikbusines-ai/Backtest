...
export default async function TradeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell>
    ...
    <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
      ...
      <div className="space-y-6"><section className="panel p-5"><div className="grid grid-cols-2 gap-x-5 gap-y-4">{details.map(([label,value]) => <div key={label}><span className="label">{label}</span><p className="text-sm font-medium text-zinc-200">{String(value)}</p></div>)}</div></section><section className="panel p-5"><span className="label">Tags</span><div className="flex flex-wrap gap-2">{trade.tags.map((tag) => <span key={tag.id} className="chip chip-active py-1.5">{tag.name}</span>)}{!trade.tags.length && <span className="text-sm text-zinc-600">Keine Tags</span>}</div></section>{(trade.context || trade.entry_note) && <section className="panel space-y-5 p-5">{trade.context && <div><span className="label">Marktkontext</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.context}</p></div>}{trade.entry_note && <div><span className="label">Warum Entry?</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.entry_note}</p></div>}</section>}{(trade.review_observation || trade.review_mistake || trade.review_invalidation) && <section className="panel space-y-5 p-5"><p className="text-sm font-semibold">Nachträgliche Auswertung</p>{trade.review_observation && <div><span className="label">Aufgefallen</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_observation}</p></div>}{trade.review_mistake && <div><span className="label">Missachtet</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_mistake}</p></div>}{trade.review_invalidation && <div><span className="label">Entry entkräftet durch</span><p className="whitespace-pre-wrap text-sm leading-6 text-zinc-400">{trade.review_invalidation}</p></div>}</section>}</div>
    </div>
  </AppShell>;
}
