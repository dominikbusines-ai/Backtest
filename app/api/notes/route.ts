...
export async function POST(request: Request) {
  ...
  try {
    const supabase = requireSupabase();
    const { data, error } = await supabase.from("journal_notes").insert(parsed.data).select(columns).single();
    ...
    if (error?.code === "23505") {
      ...
      if (existing.title === parsed.data.title && existing.content === parsed.data.content) return NextResponse.json({ note: existing });
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { data, error } = await requireSupabase().from("journal_notes").update({ ...changes, updated_at: new Date().toISOString() })
