...
export function TradeImageViewer({ src, alt, compact = false }: { src: string; alt: string; compact?: boolean }) {
  ...
  return <>
    {compact ? <button type="button" onClick={() => setOpen(true)} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/15 bg-ink/90 px-3 py-2 text-xs font-semibold text-zinc-200 hover:bg-zinc-800" aria-label={`${alt} groß ansehen`}><Maximize2 className="h-4 w-4" /> Groß ansehen</button> : <button type="button" onClick={() => setOpen(true)} className="group relative block min-h-52 w-full cursor-zoom-in overflow-hidden text-left sm:min-h-72" aria-label={`${alt} groß ansehen`}>
    ...
    {open && createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={`${alt} – große Ansicht`} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
