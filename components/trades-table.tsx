...
export function TradesTable() {
  ...
  const filtered = useMemo(() => trades.filter((trade) => {
    ...
    if (tradeMode && trade.trade_mode !== tradeMode) return false;
  ...
  return <div className="space-y-5">
    <section className="panel p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-9">
        ...
        <label><span className="label">Typ</span><select className="field" value={tradeMode} onChange={(e) => setTradeMode(e.target.value as TradeMode | "")}><option value="">Alle</option><option value="backtest">Backtest</option><option value="live">Live Trade</option></select></label>
