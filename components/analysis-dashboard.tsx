...
export function AnalysisDashboard() {
  ...
  const tagStats = useMemo(() => tags.map((tag) => ({ tag, stats: calculateStats(trades.filter((trade) => hasEveryTag(trade, [tag.id]))) })).filter((row) => row.stats.trades + row.stats.noTrades > 0).sort((a,b) => (b.stats.trades + b.stats.noTrades) - (a.stats.trades + a.stats.noTrades)), [trades, tags]);
  ...
  return <div className="space-y-8">
    ...
    <section><div className="mb-4 flex items-end justify-between"><div><p className="text-sm font-semibold">{selected.length ? "Gefilterte Kennzahlen" : "Gesamtübersicht"}</p><p className="mt-1 text-xs text-zinc-600">„Kein Trade“ wird separat gezählt und fließt nicht in Winrate oder R-Kennzahlen ein.</p></div><p className="text-xs text-zinc-600">{filtered.length} von {trades.length} Einträgen</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
