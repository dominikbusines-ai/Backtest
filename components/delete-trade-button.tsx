...
export function DeleteTradeButton({ id }: { id: string }) {
  ...
  return <button onClick={() => void remove()} disabled={loading} className="flex min-h-11 items-center justify-center gap-2 rounded-lg border border-rose-500/20 px-3 py-2 text-xs font-semibold text-rose-400 hover:bg-rose-500/5 disabled:opacity-50">{loading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />} Löschen</button>;
}
