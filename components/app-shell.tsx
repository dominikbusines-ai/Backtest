        ...
        <div className="mx-auto flex h-14 max-w-[1440px] items-center justify-between px-4 sm:h-16 sm:px-5 lg:px-8">
          ...
          <nav className="hidden items-center gap-1 sm:flex" aria-label="Hauptnavigation">
            {nav.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link key={href} href={href} aria-current={active ? "page" : undefined} className={`flex min-h-11 items-center gap-2 rounded-lg px-2 py-2 text-xs font-medium transition lg:px-4 ${active ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-200"}`}>
