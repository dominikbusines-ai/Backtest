...
export function SettingsManager() {
  ...
  if (error && !tags.length) return <div className="panel p-5 sm:p-8"><p className="text-sm font-semibold">Supabase-Verbindung erforderlich</p><p className="mt-2 text-sm text-zinc-500">{error}</p></div>;

  return <div className="grid gap-6 lg:grid-cols-[1fr_2fr]">
    <section className="panel self-start p-4 sm:p-5"><div className="mb-5"><p className="text-sm font-semibold">Kategorien</p><p className="mt-1 text-xs text-zinc-600">Frei erstellen, umbenennen und löschen.</p></div><div className="mb-4 grid grid-cols-[minmax(0,1fr)_48px] gap-2"><input className="field" placeholder="Neue Kategorie" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void addCategory(); }} /><button aria-label="Kategorie hinzufügen" onClick={() => void addCategory()} className="flex min-h-11 items-center justify-center rounded-lg bg-lime text-ink"><FolderPlus className="h-4 w-4" /></button></div><div className="divide-y divide-line">{categories.map((category) => <div key={category.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{category.name}</p><p className="mt-0.5 text-[10px] text-zinc-600">{tags.filter((tag) => tag.category_id === category.id).length} Tags</p></div><div className="flex shrink-0 gap-1"><button aria-label="Kategorie umbenennen" onClick={() => void rename("categories", category.id, category.name)} className="flex min-h-11 min-w-11 items-center justify-center rounded text-zinc-600 hover:text-white"><Pencil className="h-4 w-4" /></button><button aria-label="Kategorie löschen" onClick={() => void remove("categories", category.id, category.name)} className="flex min-h-11 min-w-11 items-center justify-center rounded text-zinc-600 hover:text-rose-400"><Trash2 className="h-4 w-4" /></button></div></div>)}</div></section>
    <section className="panel p-4 sm:p-5"><div className="mb-5"><p className="text-sm font-semibold">Tags</p><p className="mt-1 text-xs text-zinc-600">Standardwerte sind vollständig veränderbar.</p></div><div className="mb-5 grid gap-2 sm:grid-cols-[1fr_220px_auto]"><input className="field" placeholder="Neuer Tag" value={newTag} onChange={(e) => setNewTag(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void addTag(); }} /><select className="field" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}><option value="">Ohne Kategorie</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><button onClick={() => void addTag()} className="flex min-h-11 items-center justify-center gap-2 rounded-lg bg-lime px-4 text-xs font-bold text-ink"><Plus className="h-4 w-4" /> Hinzufügen</button></div>{error && <p className="mb-4 text-xs text-rose-400">{error}</p>}<div className="divide-y divide-line">{tags.map((tag) => <div key={tag.id} className="grid items-center gap-3 py-3 sm:grid-cols-[1fr_220px_auto]"><div className="min-w-0"><p className="truncate text-sm font-medium">{tag.name}</p>{tag.is_default && <p className="mt-0.5 text-[10px] uppercase tracking-wider text-zinc-700">Startwert</p>}</div><select aria-label={`Kategorie für ${tag.name}`} className="field sm:h-9" value={tag.category_id ?? ""} onChange={(e) => void assign(tag, e.target.value)}><option value="">Ohne Kategorie</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><div className="flex justify-end gap-1"><button aria-label="Tag umbenennen" onClick={() => void rename("tags", tag.id, tag.name, tag.category_id)} className="flex min-h-11 min-w-11 items-center justify-center rounded text-zinc-600 hover:text-white"><Pencil className="h-4 w-4" /></button><button aria-label="Tag löschen" onClick={() => void remove("tags", tag.id, tag.name)} className="flex min-h-11 min-w-11 items-center justify-center rounded text-zinc-600 hover:text-rose-400"><Trash2 className="h-4 w-4" /></button></div></div>)}</div>
    </section>
  </div>;
}
